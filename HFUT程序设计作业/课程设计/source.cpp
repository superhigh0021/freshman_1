...
void getline(FILE* pfile,char* pt) {
	while (!feof(pfile)) {   //这个作者坏的狠，用函数更新文件指针的指向位置
		*pt = fgetc(pfile);                         //fgetc()从文件指针指向的文件中读取一个字符，读取后，光标后移一个字符
		if (*pt=='\n') {	                        //成功读取完一行，直到换行符
			*pt = '\0';
			return;
		}
...
int Iscom(char* pt) {
	...
	pt3 = mystrstr(pt, "*/");
	if (com_flag == 0) {
		if ((pt2!=NULL) && (pt3 == NULL))	//如果只有/*表示注释开始
			com_flag = 1;
		if (pt1 == pt)	//双斜杠开头的注释
			return 1;
		else if (pt1 - pt > 0 || pt2 - pt > 0)	//有注释符号，但是不是顶头开始
			return 2;
		else if ((pt2 == pt) && ((pt3 != NULL && (*(pt3 + 2) == '\0')) || (pt3 == NULL)))	//以/*开头以*/结尾，或者以/*开头没有检测到*/
			return 1;
